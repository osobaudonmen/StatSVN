...
- **すべてのソースコードは LF（Unix形式）で統一する**
- Windows上で開発している場合も、LFを使用すること
- `.gitattributes` により改行コードは自動的に管理される
- エディタの設定は `.editorconfig` で統一される
- **既存ファイルの改行コード変更のみを目的とした変更は避ける**（別途改行コード統一の指示を受けた場合を除く）
